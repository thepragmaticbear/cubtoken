# cubtoken MVP Implementation Plan (Phases 1–3)
...
**Goal:** Ship `ctk`, a single Rust binary installed as a Claude Code PostToolUse hook that compresses `Read`/`Grep`/`Glob` (and optionally `Bash`) tool outputs 50–90% before they enter model context, failing open on any error.

**Architecture:** Cargo workspace with four crates: `ctk-cli` (clap entrypoint), `ctk-hook` (hook protocol + dispatch, host-agnostic), `ctk-compress` (pure compressor functions), `ctk-sitter` (tree-sitter → signature skeleton). The hook reads PostToolUse JSON on stdin and either emits `{hookSpecificOutput: {updatedToolOutput: …}}` on stdout or emits nothing (pass-through). See companion spec: `2026-06-12-cubtoken-design.md`.
...
1. **Fail open** — any error in `ctk hook` → print nothing, exit 0.
...
- Create: `crates/ctk-cli/Cargo.toml`, `crates/ctk-cli/src/main.rs`
- Create: `crates/ctk-hook/Cargo.toml`, `crates/ctk-hook/src/lib.rs`
- Create: `crates/ctk-compress/Cargo.toml`, `crates/ctk-compress/src/lib.rs`
- Create: `crates/ctk-sitter/Cargo.toml`, `crates/ctk-sitter/src/lib.rs`
...
[workspace]
resolver = "2"
members = ["crates/ctk-cli", "crates/ctk-hook", "crates/ctk-compress", "crates/ctk-sitter"]
...
**Files:**
- Modify: `crates/ctk-cli/src/main.rs`
- Create: `tests/fixtures/` (recorded payloads, sanitized)

- [x] **Step 1: Write failing CLI test** in `crates/ctk-cli/tests/record.rs` (add `assert_cmd = "2"`, `tempfile = "3"` as dev-deps):
...
- [x] **Step 2: Run — verify FAIL** (`cargo test -p ctk-cli`): no `record` subcommand. ✅ failed as expected.
...
**Files:**
- Create: `crates/ctk-hook/src/protocol.rs`
- Modify: `crates/ctk-hook/src/lib.rs`, `crates/ctk-cli/src/main.rs`

- [x] **Step 1: Write failing tests** in `crates/ctk-hook/src/protocol.rs` (unit tests parse every fixture):
...
`lib.rs` exposes `pub fn run_hook(stdin: &str, cfg: &Config) -> Option<String>`: parse → dispatch (returns `None` for every tool for now) → serialize. `Config::default()` for now is an empty struct (fleshed out in Task 5). In `ctk-cli`, `Hook` reads stdin, wraps the call in `std::panic::catch_unwind`, prints `Some(json)` or nothing, **always exits 0**; on error, appends a line to `.cubtoken/errors.log` if writable, never to stdout/stderr.

- [x] **Step 4: Run — verify PASS** (`cargo test -p ctk-hook`).

- [x] **Step 5: Commit** — `git commit -m "Hook protocol types and fail-open runner"`

---

### Task 4: tree-sitter signature skeleton (`ctk-sitter`)

**Files:**
- Modify: `crates/ctk-sitter/src/lib.rs`
- Create: `crates/ctk-sitter/tests/skeleton.rs`, `crates/ctk-sitter/tests/corpus/sample.rs` (a ~200-line realistic Rust file with structs, impls, doc comments, private fns)
...
### Task 5: Config loading (`ctk-compress`)

**Files:**
- Create: `crates/ctk-compress/src/config.rs`
- Modify: `crates/ctk-compress/src/lib.rs` (add `toml = "0.8"` dep)
...
- [x] **Step 3: Implement** `Config` with serde-default structs exactly mirroring the spec §5 TOML (`read.{enabled, threshold_tokens, never_compress}`, `grep.{enabled, max_matches_per_file}`, `glob.{enabled, max_paths}`, `bash.enabled`, `stats.ledger`). `load_from(global: Option<&str>, project: Option<&str>)` parses each TOML and field-wise overlays project over global over defaults (each field is `Option` in the raw deserialization layer, then resolved). `load()` reads `~/.config/cubtoken/config.toml` and `<cwd>/.cubtoken.toml`. `is_excluded` builds a `globset::GlobSet` (cached with `OnceCell`). Move the placeholder `Config` from Task 3 here; `ctk-hook` re-exports it.
...
**Files:**
- Create: `crates/ctk-compress/src/read.rs`, `crates/ctk-compress/src/estimate.rs`
- Modify: `crates/ctk-hook/src/lib.rs` (dispatch)
- Create: `crates/ctk-hook/tests/end_to_end.rs`
...
```rust
// crates/ctk-hook/tests/end_to_end.rs
...
fn large_read_is_compressed_with_escape_hatch() {
    ...
    assert!(text.contains("cubtoken"));            // banner present
...
- [x] **Step 3: Implement.**
  ...
  - `read.rs`: `pub fn compress_read(input: &Value, response: &Value, cfg: &Config) -> Option<Value>`:
    ...
    4. `lang_for_path` + `skeleton()`; fallback when `None`: head 40 lines + `… [cubtoken: lines 41-N elided] …` + tail 20 lines, all with line-number gutter.
    5. Banner: `[cubtoken: compressed view of {path} — {orig_chars} chars → skeleton. This is NOT the full file. Before quoting or editing, Read the exact region: Read(file_path={path}, offset=<line>, limit=<count>). Bracketed [La-Lb] ranges mark elided bodies.]`
  ...
  - `ctk-hook` dispatch: `tool_name == "Read"` → `compress_read`; wrap result in `HookOutput`.
...
**Files:**
- Create: `crates/ctk-hook/src/ledger.rs`
- Modify: `crates/ctk-hook/src/lib.rs`, `crates/ctk-cli/src/main.rs` (hook matcher will include Edit/Write)
...
### Task 8: `ctk init` and `ctk doctor`

**Files:**
- Create: `crates/ctk-cli/src/init.rs`, `crates/ctk-cli/src/doctor.rs`
- Modify: `crates/ctk-cli/src/main.rs`
- Create: `crates/ctk-cli/tests/init.rs`
...
fn init_installs_posttooluse_hook() {
    ...
    assert!(hook["hooks"][0]["command"].as_str().unwrap().contains("ctk hook"));
}
...
fn init_is_idempotent_and_preserves_existing_settings() {
    ...
    for _ in 0..2 {
        Command::cargo_bin("ctk").unwrap().current_dir(dir.path()).arg("init").assert().success();
    }
    let s: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(dir.path().join(".claude/settings.json")).unwrap()).unwrap();
...
- [x] **Step 3: Implement** `init.rs`: read-modify-write `.claude/settings.json` (`--global` → `~/.claude/settings.json`): parse as `Value`, ensure `hooks.PostToolUse` array, remove any prior entry whose command contains `"ctk hook"`, append ours (`command: "ctk hook"` — resolved to the absolute binary path via `std::env::current_exe()`), pretty-print back. Also write a starter `.cubtoken.toml` if absent. `doctor.rs`: checks and prints PASS/FAIL lines for: hook entry present; `ctk` on PATH or absolute path valid; `.cubtoken/` writable; rtk present on PATH (informational: "bash compression deferred to rtk"). Exit 1 if any FAIL.

- [x] **Step 4: Run — verify PASS.**

- [x] **Step 5: Commit** — `git commit -m "ctk init and ctk doctor"`
...
**Files:**
- Create: `crates/ctk-cli/src/stats.rs`
- Modify: `crates/ctk-cli/src/main.rs`

- [x] **Step 1: Write failing test** (assert_cmd; seed a `.cubtoken/` with two session JSONL files via `Ledger` API or literal lines):

```rust
#[test]
fn stats_reports_totals_and_percent() {
    // seeded: Read in=14000 out=2400 across two sessions
    let assert = Command::cargo_bin("ctk").unwrap()
...
- [x] **Step 3: Implement:** glob `.cubtoken/session-*.jsonl`, replay `save` records, print a per-tool table (tokens in, tokens out, saved, %) plus a lifetime total row.

- [x] **Step 4: Run — verify PASS.**

- [x] **Step 5: Commit** — `git commit -m "ctk stats savings report"`

**🏁 Phase 1 complete — dogfood checkpoint:** run `ctk init` in this repo, develop the remaining tasks with cubtoken active, and check `ctk stats` daily. If skeleton views confuse the agent in practice (failed Edits, repeated re-reads), fix before proceeding.
...
**Files:**
- Create: `crates/ctk-compress/src/grep.rs`
- Modify: `crates/ctk-hook/src/lib.rs` (dispatch), `crates/ctk-hook/tests/end_to_end.rs`
...
**Files:**
- Create: `crates/ctk-compress/src/glob_fold.rs`
- Modify: `crates/ctk-hook/src/lib.rs`, `crates/ctk-hook/tests/end_to_end.rs`
...
**Files:**
- Create: `crates/ctk-compress/src/bash.rs`
- Modify: `crates/ctk-hook/src/lib.rs`, `crates/ctk-cli/src/doctor.rs`
...
- [x] **Step 2:** README: what it does (one paragraph), the verified mechanism (PostToolUse `updatedToolOutput`), install (`cargo install`, `ctk init`), the four invariants from this plan's header, rtk coexistence note, config reference (spec §5 table).
