...
**Status.** `safe` has not been validated. The events that attribution depends on have been confirmed live on Claude Code 2.1.280: in a real session, a compressed Read followed by a targeted Read of its elided lines was recorded as a high-confidence recovery. It hasn't been shown that the governor reduces total token use. Leave it off unless you're evaluating it. The evaluation contract is [ADAPTIVE_GOVERNOR_EVALUATION_SPEC.md](ADAPTIVE_GOVERNOR_EVALUATION_SPEC.md), and the current verdict and measurements are in [tests/evaluation/RESULTS.md](tests/evaluation/RESULTS.md). On one Apple M5, safe mode added about 0.17 ms p95 per compressed Read, and the per-turn rebuild took about 6 ms at 10,000 ledger records, which is over its 5 ms target.

**Worktrees.** Claude Code sessions that run in a per-session git worktree don't see an untracked `.cubtoken.toml` at the repository root, and each worktree keeps its own `.cubtoken/`. The governor then runs with defaults, its observations don't accumulate across sessions, and they are deleted along with the worktree. Put the `[adaptive]` setting in `~/.config/cubtoken/config.toml`, or commit `.cubtoken.toml`.
