//! `stk stats`: replay every session ledger under `.smalltoke/` and print a
//! per-tool savings table plus lifetime totals.

use stk_hook::ledger::{Ledger, Totals};

pub fn run() {
    let mut per_tool: std::collections::BTreeMap<String, Totals> = Default::default();

    if let Ok(entries) = std::fs::read_dir(".smalltoke") {
        for entry in entries.flatten() {
            ...
            let ledger = Ledger::open(std::path::Path::new(".smalltoke"), session);
    ...
    if per_tool.is_empty() {
        println!("no savings recorded yet — is the hook installed? (stk doctor)");
        return;
    }
