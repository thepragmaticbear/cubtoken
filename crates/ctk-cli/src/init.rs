...
pub fn is_ctk_hook_command(cmd: &str) -> bool {
    cmd.ends_with("ctk hook") || cmd.contains("ctk\" hook") || cmd.contains("ctk' hook")
}
...
fn write_starter_config(path: &Path) {
    ...
    let starter = "\
# cubtoken project config — see https://github.com/brandon/cubtoken
[read]
enabled = true
