...
pub fn compress_read(
    ...
    let compressed = format!(
        "[smalltoke: compressed view of {file_path} — {} chars → skeleton. \
...
fn skeleton_view(content: &str, file_path: &str) -> Option<String> {
    let lang = stk_sitter::lang_for_path(file_path)?;
    Some(stk_sitter::skeleton(content, lang)?.rendered)
}
...
mod tests {
    ...
    const SAMPLE: &str = include_str!("../../stk-sitter/tests/corpus/sample.rs");
    ...
    fn exactly_one_banner() {
        ...
        let out = compress_read(&input, &response, &low_threshold()).unwrap();
        let compressed = out.updated_response["file"]["content"].as_str().unwrap();
        assert_eq!(compressed.matches("[smalltoke:").count(), 1);
    }
